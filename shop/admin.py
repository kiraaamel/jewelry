...
from django.db.models import QuerySet, Sum, Count, Model
...
class ReviewAdmin(admin.ModelAdmin):
    ...
    Attributes:
        ...
        list_editable: Редактируемые поля прямо в списке.
        readonly_fields: Только для чтения.
        actions: Доступные действия.
        fieldsets: Группировка полей в форме редактирования.
    """
    list_display: list[str] = ['id', 'user', 'product', 'rating', 'created_at', 'moderated']
    list_filter: list[str] = ['rating', 'moderated', 'created_at']
    search_fields: list[str] = ['user__email', 'product__name', 'comment']
    list_editable: list[str] = ['moderated']
    readonly_fields: list[str] = ['created_at']

    actions: list[str] = ['mark_as_moderated']
    ...
    def mark_as_moderated(self, request: HttpRequest, queryset: QuerySet[Review]) -> None:
        ...
        Args:
            request: HTTP-запрос.
            queryset: QuerySet выбранных отзывов.
        """
        updated: int = queryset.update(moderated=True)
        self.message_user(request, f'{updated} отзывов отмечено как промодерированные')


class OrderItemInline(admin.TabularInline):
    """
    Inline-форма для управления позициями заказа прямо на странице заказа.

    Attributes:
        model: Связанная модель OrderItem.
    ...
    extra: int = 1
    fields: list[str] = ['product', 'variant', 'quantity', 'price', 'item_total_display']
    readonly_fields: list[str] = ['item_total_display']
    ...
    def item_total_display(self, obj: OrderItem) -> str:
        ...
        Args:
            obj: Экземпляр OrderItem.

        Returns:
            Форматированная строка с общей стоимостью позиции.
        """
        if obj.pk and obj.price and obj.quantity:
            total: Decimal = obj.price * obj.quantity
            return f"{total} ₽"
        return "0 ₽"
    item_total_display.short_description: str = "Стоимость"
    ...
    def formfield_for_foreignkey(self, db_field: ForeignKey, request: HttpRequest, **kwargs: Any) -> Any:
        ...
        Args:
            db_field: Поле внешнего ключа.
            request: HTTP-запрос.
            **kwargs: Дополнительные аргументы.

        Returns:
            Настроенное поле формы.
        """
        if db_field.name == "variant":
            object_id: Optional[str] = request.resolver_match.kwargs.get('object_id')
            if object_id:
                order_id: int = int(object_id)
...
class OrderItemAdmin(admin.ModelAdmin):
    ...
    """
    list_display: list[str] = ['id', 'order', 'product', 'variant', 'product_name', 'size', 'quantity', 'price', 'item_total_display']
    list_filter: list[str] = ['order__created_at']
    search_fields: list[str] = ['product__name', 'product_name', 'order__order_number', 'variant__size']
    readonly_fields: list[str] = ['item_total_display']
    ordering: list[str] = ['-id']
    ...
    def size(self, obj: OrderItem) -> str:
        """
        Отображает размер из варианта товара.

        Args:
            obj: Экземпляр OrderItem.

        Returns:
            Размер товара или пустая строка.
        """
        return obj.variant.size if obj.variant else obj.size
    size.short_description: str = 'Размер'
...
class OrderAdmin(admin.ModelAdmin):
    ...
    Attributes:
        ...
        readonly_fields: Только для чтения.
        list_editable: Редактируемые поля прямо в списке.
        exclude: Исключённые поля.
        date_hierarchy: Иерархия дат.
        actions: Доступные действия.
        fieldsets: Группировка полей в форме редактирования.
    """
    inlines: list[type[admin.TabularInline]] = [OrderItemInline]
    list_display: list[str] = [
        'order_number', 'user', 'created_at', 'status',
        'total_price_display', 'delivery_method', 'delivery_date', 'delivery_time', 'pickup_code'
    ]
    list_filter: list[str] = ['status', 'delivery_method', 'payment_method', 'created_at']
    search_fields: list[str] = ['order_number', 'user__email', 'delivery_address']
    readonly_fields: list[str] = ['order_number', 'created_at', 'total_price_display', 'pickup_code', 'code_generated_at']
    list_editable: list[str] = ['status']
    exclude: list[str] = ['total_price']
    date_hierarchy: str = 'created_at'

    actions: list[str] = ['generate_order_report_pdf', 'export_orders_csv']
    ...
    def total_price_display(self, obj: Order) -> str:
        ...
        Returns:
            Форматированная строка с общей стоимостью заказа.
        """
        if obj.pk:
            ...
            return f"{total} ₽"
        return "0 ₽"
    total_price_display.short_description: str = "Общая стоимость"
    ...
    def save_model(self, request: HttpRequest, obj: Order, form: Any, change: bool) -> None:
        ...
        """
        if change:
            original: Order = Order.objects.get(pk=obj.pk)
    ...
    def generate_order_report_pdf(self, request: HttpRequest, queryset: QuerySet[Order]) -> Optional[HttpResponse]:
        """
        Генерирует PDF отчёт по выбранным заказам используя reportlab.

        Args:
            request: HTTP-запрос.
            queryset: QuerySet выбранных заказов.

        Returns:
            HttpResponse с PDF файлом или None при ошибке.
        """
        ...
        total_sum: Decimal = queryset.aggregate(total=Sum('total_price'))['total'] or Decimal('0')
        ...
        for order in queryset:
            ...
            items_data: list[list[str]] = [["Товар", "Кол-во", "Цена", "Сумма"]]
            for item in order.items.all():
                items_data.append([
    ...
    def export_orders_csv(self, request: HttpRequest, queryset: QuerySet[Order]) -> Optional[HttpResponse]:
        """
        ...
        Returns:
            HttpResponse с CSV файлом или None при ошибке.
        """
        import csv
    ...
    def get_delivery_method_ru(self, method: str) -> str:
        ...
        Args:
            method: Английский идентификатор способа доставки.

        Returns:
            Русское название способа доставки.
        """
    ...
    def get_payment_method_ru(self, method: str) -> str:
        ...
        Args:
            method: Английский идентификатор способа оплаты.
...
class CartItemInline(admin.TabularInline):
    """
    Inline-форма для управления позициями корзины.

    Attributes:
        model: Связанная модель CartItem.
    ...
    extra: int = 0
    fields: list[str] = ['product', 'variant', 'quantity', 'added_at', 'total_price']
    readonly_fields: list[str] = ['added_at', 'total_price']
    ...
    def formfield_for_foreignkey(self, db_field: ForeignKey, request: HttpRequest, **kwargs: Any) -> Any:
        ...
        if db_field.name == "variant":
            object_id: Optional[str] = request.resolver_match.kwargs.get('object_id')
            if object_id:
                cart_id: int = int(object_id)
                cart: Optional[Cart] = Cart.objects.filter(id=cart_id).first()
...
class CartAdmin(admin.ModelAdmin):
    ...
    list_display: list[str] = ['id', 'user', 'session_key', 'total_items', 'total_price', 'updated_at']
    list_filter: list[str] = ['updated_at', 'created_at']
    search_fields: list[str] = ['user__email', 'session_key']
...
class CartItemAdmin(admin.ModelAdmin):
    """
    Настройки отображения позиций корзины в админке.
    """
    list_display: list[str] = ['id', 'cart', 'product', 'variant', 'quantity', 'added_at', 'total_price']
    list_filter: list[str] = ['added_at']
    search_fields: list[str] = ['product__name', 'cart__user__email', 'cart__session_key', 'variant__size']
    readonly_fields: list[str] = ['added_at', 'total_price']
    ordering: list[str] = ['-added_at']
...
class ProductVariantInline(admin.TabularInline):
    ...
    Attributes:
        model: Связанная модель ProductVariant.
        extra: Количество пустых форм.
        fields: Отображаемые поля.
        readonly_fields: Только для чтения.
    """
    model: type[ProductVariant] = ProductVariant
    extra: int = 1
    fields: list[str] = ['size', 'stock_quantity', 'reserved_quantity', 'available_quantity_display', 'sku']
    readonly_fields: list[str] = ['available_quantity_display']

    def available_quantity_display(self, obj: ProductVariant) -> str:
        """
        Отображает доступное количество с цветовой индикацией.

        Args:
            obj: Экземпляр ProductVariant.

        Returns:
            HTML-строка с информацией о доступном количестве.
        """
        available: int = obj.available_quantity
        if available <= 0:
    ...
    available_quantity_display.short_description: str = 'Доступно'
...
class ProductAdmin(admin.ModelAdmin):
    ...
    Attributes:
        inlines: Встроенные формы для вариантов товара.
        list_display: Поля для отображения в списке.
        list_filter: Поля для фильтрации.
        list_display_links: Поля-ссылки на редактирование.
        search_fields: Поля для поиска.
        prepopulated_fields: Автозаполняемые поля.
        readonly_fields: Только для чтения.
        raw_id_fields: Поля с raw_id виджетом.
        date_hierarchy: Иерархия дат.
        actions: Доступные действия.
        fieldsets: Группировка полей в форме редактирования.
    """
    inlines: list[type[admin.TabularInline]] = [ProductVariantInline]
    list_display: list[str] = [
    ...
    list_filter: list[str] = ['category', 'silver_type', 'fineness', 'stones', 'created_at', 'collection']
    list_display_links: list[str] = ['name']
    search_fields: list[str] = ['name', 'description', 'collection__name']
    prepopulated_fields: dict[str, list[str]] = {'slug': ('name',)}
    readonly_fields: list[str] = ['created_at', 'updated_at', 'full_silver_info', 'images_preview']
    raw_id_fields: list[str] = ['created_by', 'category']
    date_hierarchy: str = 'created_at'
    ...
    actions: list[str] = ['apply_discount', 'increase_price', 'duplicate_product']
    ...
    def price_display(self, obj: Product) -> str:
        ...
        Args:
            obj: Экземпляр Product.

        Returns:
            HTML-строка с отображением цены и скидки.
        """
        if obj.has_discount:
            discount_percent: int = int((obj.old_price - obj.price) / obj.old_price * 100)
            return format_html(
    ...
    def silver_info(self, obj: Product) -> str:
        ...
        Returns:
            HTML-строка с информацией о серебре.
        """
        silver_type_display: str = obj.get_silver_type_display()
        fineness_display: str = obj.get_fineness_display()
    ...
    def full_silver_info(self, obj: Product) -> str:
        ...
        Returns:
            HTML-строка с полной информацией о серебре и размерах.
        """
        variants_info: str = ""
        if obj.variants.exists():
            sizes: list[str] = [v.size for v in obj.variants.all()]
            variants_info = f"<br><strong>Размеры:</strong> {', '.join(sizes)}"
    ...
    def image_preview(self, obj: Product) -> str:
        ...
        Returns:
            HTML-строка с тегом img или дефис.
        """
        if obj.image:
            return format_html(
    ...
    def images_preview(self, obj: Product) -> str:
        ...
        Returns:
            HTML-строка с галереей миниатюр.
        """
        images_html: str = '<div style="display: flex; gap: 10px; flex-wrap: wrap;">'
        if obj.image:
    ...
    def images_count_display(self, obj: Product) -> str:
        ...
        Returns:
            HTML-строка с цветовой индикацией количества фото.
        """
        count: int = obj.images_count
        if count == 0:
    ...
    def stock_status(self, obj: Product) -> str:
        ...
        Returns:
            Строка со статусом: 'Нет в наличии', 'Мало' или 'В наличии'.
        """
        total_available: int = obj.available_quantity
    ...
    def stones_display(self, obj: Product) -> str:
        ...
        Returns:
            Строка с информацией о камнях.
        """
        if not obj.stones:
            return 'Без камней'

        stone_display: str = obj.get_stone_type_display()
        if obj.stone_weight:
    ...
    def apply_discount(self, request: HttpRequest, queryset: QuerySet[Product]) -> None:
        ...
        Args:
            request: HTTP-запрос.
            queryset: QuerySet выбранных товаров.
        """
    ...
    def duplicate_product(self, request: HttpRequest, queryset: QuerySet[Product]) -> None:
        ...
        count: int = 0
        for product in queryset:
            product_copy: Product = copy.copy(product)
            ...
            for variant in product.variants.all():
                variant_copy: ProductVariant = copy.copy(variant)
...
class WishlistAdmin(admin.ModelAdmin):
    ...
    Attributes:
        ...
        readonly_fields: Только для чтения.
    """
    list_display: list[str] = ['id', 'user', 'product', 'added_at']
    list_filter: list[str] = ['added_at']
    search_fields: list[str] = ['user__email', 'product__name']
    readonly_fields: list[str] = ['added_at']
...
class UserAdmin(admin.ModelAdmin):
    ...
    Attributes:
        ...
        readonly_fields: Только для чтения.
    """
    list_display: list[str] = ['email', 'first_name', 'last_name', 'phone', 'birthday', 'bonus_points', 'is_staff', 'is_active']
    list_filter: list[str] = ['is_staff', 'is_active', 'is_superuser']
    search_fields: list[str] = ['email', 'first_name', 'last_name', 'phone']

    actions: list[str] = ['add_bonus_points', 'make_active', 'make_inactive']
    ...
    readonly_fields: list[str] = ['last_login', 'date_joined']
...
class PromoCodeAdmin(admin.ModelAdmin):
    ...
    Attributes:
        ...
        list_filter: Поля для фильтрации.
        search_fields: Поля для поиска.
        filter_horizontal: Поля с горизонтальным виджетом many-to-many.
        actions: Доступные действия.
        fieldsets: Группировка полей в форме редактирования.
    """
    list_display: list[str] = ['code', 'discount_display', 'min_order_amount', 'valid_from', 'valid_to', 'is_valid', 'used_count']
    list_filter: list[str] = ['discount_type', 'is_active', 'only_new_users']
    search_fields: list[str] = ['code']
    filter_horizontal: list[str] = ['applicable_categories']

    actions: list[str] = ['activate_promocodes', 'deactivate_promocodes']
    ...
    def discount_display(self, obj: PromoCode) -> str:
        ...
        Args:
            obj: Экземпляр PromoCode.

        Returns:
            Строка с отображением скидки.
        """
        return obj.discount_display
    discount_display.short_description: str = 'Скидка'
    ...
    is_valid.short_description: str = 'Активен'
    ...
    def activate_promocodes(self, request: HttpRequest, queryset: QuerySet[PromoCode]) -> None:
        ...
        Args:
            request: HTTP-запрос.
            queryset: QuerySet выбранных промокодов.
        """
        updated: int = queryset.update(is_active=True)
...
class PromoCodeUsageAdmin(admin.ModelAdmin):
    ...
    Attributes:
        ...
        readonly_fields: Только для чтения.
    """
    list_display: list[str] = ['user', 'promo_code', 'order', 'discount_amount', 'used_at']
    list_filter: list[str] = ['used_at']
    search_fields: list[str] = ['user__email', 'promo_code__code', 'order__order_number']
    readonly_fields: list[str] = ['used_at']
...
class CollectionAdmin(admin.ModelAdmin):
    ...
    Attributes:
        ...
        fieldsets: Группировка полей в форме редактирования.
    """
    list_display: list[str] = ['name', 'slug', 'order', 'is_active', 'products_count', 'image_preview']
    ...
    products_count.short_description: str = 'Товаров в коллекции'
    ...
    def image_preview(self, obj: Collection) -> str:
        ...
        return '-'
    image_preview.short_description: str = 'Превью'


class ProductVariantAdmin(admin.ModelAdmin):
    """
    Настройки отображения вариантов товаров в админке.
    ...
    """
    list_display: list[str] = ['id', 'product', 'size', 'stock_quantity', 'reserved_quantity', 'available_quantity', 'sku']
    list_filter: list[str] = ['product__category', 'size']
    search_fields: list[str] = ['product__name', 'size', 'sku']
    list_editable: list[str] = ['stock_quantity', 'reserved_quantity', 'sku']
    autocomplete_fields: list[str] = ['product']

    def available_quantity(self, obj: ProductVariant) -> int:
        """
        Возвращает доступное количество.
    ...
    available_quantity.short_description: str = 'Доступно'

    def get_queryset(self, request: HttpRequest) -> QuerySet[ProductVariant]:
        """
        Оптимизирует запросы с select_related.
